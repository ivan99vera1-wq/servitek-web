 ...
 * SplashCursor: es una simulación en WebGL, no tiene sentido tiparla a mano.
 * Única diferencia respecto al original: el CSS de una sola clase
 * (`.ghost-fibers-container`) se aplicó como clases de Tailwind en el
 * `className` del contenedor en vez de un archivo `.css` aparte.
 */
const hexToRgb = hex => {
...
const GhostFibers = ({
  ...
  useEffect(() => {
    ...
    // WebGL2 (y WebGL a secas) puede no estar disponible: deshabilitado por
    // el usuario, política de la empresa o un dispositivo/driver sin
    // soporte. `ogl` no lo maneja -su Renderer revienta con un TypeError si
    // ambos contextos vienen null- y este componente vive en el layout raíz,
    // así que sin este try/catch un visitante sin WebGL vería caer *todo*
    // el sitio contra el error boundary global por un fondo decorativo.
    try {
      return setupGhostFibers();
    } catch (error) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn('GhostFibers: WebGL no disponible, se omite el fondo decorativo.', error);
      }
      return undefined;
    }

    function setupGhostFibers() {
